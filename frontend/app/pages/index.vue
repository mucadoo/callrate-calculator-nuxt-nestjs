<template>
  <div>
    ...
    <div v-if="results.length > 0" class="result-container">
      <a-row :gutter="24">
        <a-col :span="24" v-for="res in results" :key="res.provider" style="margin-bottom: 16px">
          <a-card :title="res.provider">
            <template #extra>
              <a-tag color="blue">Standard Rate: ${{ res.standardRate }}/min</a-tag>
            </template>
            <p><strong>Standard Cost:</strong> ${{ res.standardCost }}</p>
            <div v-if="res.plans.length > 0">
              <p><strong>Available Plans:</strong></p>
              <a-list size="small" :data-source="res.plans">
                <template #renderItem="{ item }">
                  <a-list-item>
                    {{ item.planName }}: <strong>${{ item.costWithPlan }}</strong>
                  </a-list-item>
                </template>
              </a-list>
            </div>
            <div v-else>
              <p>No calling plans available for this provider.</p>
            </div>
          </a-card>
        </a-col>
      </a-row>
